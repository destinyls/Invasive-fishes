...
MODEL_PATH = "/home/yanglei/QWen/Qwen2.5-VL-3B-Instruct"
BSZ = 32
...
for dataset_name in ['Video-R1-260k']:
    ...
    def normalize_number(num_str):
        try:
            ...
            import re
            # Find the first number in the string (including decimals and negative signs)
    ...
    for i in tqdm(range(start_idx, len(messages), BSZ), desc="Processing batches"):
        ...
        try:
            ...
            llm_inputs = []
            valid_indices = []  # Track which samples were processed successfully
            ...
            for idx, prompt in enumerate(prompts):
                ...
                try:
                    if mm_type == 'image':
                        ...
                        # Try processing the video up to 3 times
                        max_retries = 3
                        for retry in range(max_retries):
                            try:
                                ...
                                if retry == max_retries - 1:  # Last retry
                                    print(f'Failed to process video after {max_retries} attempts for sample {data[i+idx]["path"]}')
                                    print(f'Error details: {str(e)}')
                                    raise  # Re-raise the exception to be caught by outer try-except
                                print(f'Retry {retry + 1}/{max_retries} for video {data[i+idx]["path"]}')
                                continue
                    
                    # If we get here, the sample was processed successfully
                    ...
                    print(f'Error processing media for sample {data[i+idx]["path"]}: {str(e)}')
                    # Skip this sample and continue with the next one
                    continue

            if not llm_inputs:  # If all samples in the batch failed
                print(f'All samples in batch starting at index {i} failed processing')
                batch_output_text = ['<answer>error</answer>'] * BSZ
            else:
                outputs = llm.generate(llm_inputs, sampling_params=sampling_params)
                # Initialize with error messages
                batch_output_text = ['<answer>error</answer>'] * BSZ
                # Fill in successful outputs at their original positions
                for valid_idx, output in zip(valid_indices, outputs):
                    batch_output_text[valid_idx] = output.outputs[0].text
            
        except Exception as e:
            print(f'Batch processing error at index {i}: {str(e)}')
